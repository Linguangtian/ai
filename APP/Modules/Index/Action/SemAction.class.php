		 ...
		 public function regSemsw(){
		  ...
		  public function regSemposty(){
			 ...
			 if (IS_AJAX) {
				...
				$data['username']      =  I('post.mobile','','strval');
				$code = I('post.code','');
				...
				$count = M('member')->where(array('username'=>trim($data['username'])))->count();
				if ($count>0) {
					$this->ajaxReturn(array('result'=>0,'info'=>'用户已存在！'));
				}	
				if (M('member')->where(array('mobile'=>trim($data['username'])))->getField('id')) {
                ...
                M('member')->where(array('id'=>$parent_id))->setInc('direct_member_num');
